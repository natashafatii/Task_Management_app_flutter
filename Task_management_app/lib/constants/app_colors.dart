import 'package:flutter/material.dart';

class AppColors {
  // Primary theme colors (used throughout the app, login included)
  static const Color primary = Color(0xFF3F51B5);       // Indigo, used in login icon, buttons
  static const Color primaryDark = Color(0xFF1A237E);   // Dark Indigo, used in heading text
  static const Color primaryLight = Color(0xFFC5CAE9);  // Light Indigo
  static const Color secondary = Color(0xFF5C6BC0);     // Medium Indigo, focused borders, links
  static const Color accent = Color(0xFFE91E63);        // Pink accent

  // Backgrounds / surfaces
  static const Color background = Color(0xFFE8EAF6);    // Light gradient top / general background
  static const Color backgroundBottom = Color(0xFFFFFFFF); // Gradient bottom / surface
  static const Color surface = Color(0xFFFFFFFF);       // Cards / input fields
  static const Color surfaceDark = Color(0xFF1E1E1E);   // Dark surface

  // Text colors
  static const Color textPrimary = Color(0xFF000000);   // Black for main text
  static const Color textSecondary = Color(0xFF424242); // Dark grey for subtitles / helpers
  static const Color textLight = Color(0xFFFFFFFF);     // White text (dark mode / buttons)

  // Icons / input field icons
  static const Color iconPrimary = Color(0xFF3F51B5);   // Login icon
  static const Color iconGrey = Color(0xFF90A4AE);      // Input field icons

  // Buttons
  static const Color loginButtonBg = Color(0xFF3F51B5); // Login button background
  static const Color loginButtonText = Color(0xFFFFFFFF);

  // Social buttons
  static const Color googleRed = Color(0xFFDB4437);
  static const Color facebookBlue = Color(0xFF3B5998);

  // Status colors
  static const Color success = Color(0xFF4CAF50);
  static const Color warning = Color(0xFFFFC107);
  static const Color error = Color(0xFFF44336);
  static const Color info = Color(0xFF29B6F6);

  // NEW COLORS ADDED FOR DARK MODE SUPPORT
  static const Color primaryContainer = Color(0xFF5C6BC0);     // For debug buttons & secondary actions
  static const Color onPrimaryContainer = Color(0xFFFFFFFF);   // Text on primaryContainer
  static const Color surfaceVariant = Color(0xFF2D2D2D);       // Alternative surface color
  static const Color onSurfaceVariant = Color(0xFFCCCCCC);     // Text on surface variant
  static const Color outline = Color(0xFF5C6BC0);              // Border/outline color
  static const Color outlineVariant = Color(0xFF424242);       // Subtle borders

  // Dark mode specific text colors
  static const Color textDarkPrimary = Color(0xFFFFFFFF);      // Primary text in dark mode
  static const Color textDarkSecondary = Color(0xFFB0B0B0);    // Secondary text in dark mode
  static const Color textDarkTertiary = Color(0xFF888888);     // Tertiary text in dark mode

  // Additional utility colors
  static const Color dividerLight = Color(0xFFE0E0E0);         // Divider in light mode
  static const Color dividerDark = Color(0xFF424242);          // Divider in dark mode

  // Container colors for different states
  static const Color errorContainer = Color(0xFFFFCDD2);       // Light error background
  static const Color onErrorContainer = Color(0xFFB71C1C);     // Text on error container
  static const Color successContainer = Color(0xFFC8E6C9);     // Light success background
  static const Color onSuccessContainer = Color(0xFF1B5E20);   // Text on success container
}